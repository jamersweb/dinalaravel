<template lang="">
    <div class="login_cont">
        <img src="/cms-assets/images/login/logo.png" alt="" class="logo-img">
        <div class="lgn-form">
            ...
            <form>
                ...
                <div class="mt-1" v-if="mailErr">
                    <p class="text-danger h7">*Please fill this field in correct format</p>
                </div>
                ...
                <div class="mt-1" v-if="passErr">
                    <p class="text-danger h7">*Please fill this field</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        login(e) {
            ...
            axios.post(config.baseApiUrl + 'login', {
                ...
                if (response.status == false) {
                    ...
                    config.storage.setItem('fwd_session_token', response.userdata.api_token);
                    // Try to emit showBarsEvent, but don't fail if parent doesn't handle it
                    try {
                        this.$emit('showBarsEvent');
                    } catch (e) {
                        // Ignore if event handler doesn't exist
                    }
        ...
        loggedIn() {
            // Check if user is logged in, but don't auto-redirect
            // This method is kept for potential future use but won't redirect
            const token = config.storage.getItem('fwd_session_token');
            return token != null;
        }
    },
    mounted() {
        // Always show login page when navigating to /user/login
        // Don't auto-redirect even if user has a token - let them see the login page
        // Try to emit hideBarsEvent, but don't fail if parent doesn't handle it
        try {
...
.login_cont {
    background-image: url('/cms-assets/images/login/bgrnd.png');

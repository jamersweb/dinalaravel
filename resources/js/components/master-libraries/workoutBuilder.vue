<template lang="">
    <div class="my-popup-component" @click.self="showConfirmModal">
        ...
        <div class="main-box position-relative px-2 px-md-4 py-4">
            ...
            <div class="gray_bg py-2 px-4 me-4">
                <div class="d-flex justify-content-between flex-wrap">
                    <div class="" style="max-width:100%">
                        <!-- <span class="fw-bold me-4">{{workout.type}}</span> -->
                        <input disabled class="fake_textf" :value="workout.title" style="max-width:100%">
                        <span class="mb-0 mx-4">({{workout.language=='en'?'English':(workout.language=='ar'?'Arabic':'No Audio')}})</span>
                    </div>
                    <div class="mt-2 mt-md-0">
                        <button @click="validate()" class="prim_btn px-5 text-white" style="border-radius:10px">Save</button>
                    </div>
                </div>
            </div>
            <div class="row mt-4">
                <div class="col-xl-6 exercises">
                    <div class="instruc">
                        ...
                        <textarea @change="this.instructionError=false" v-model="instructions" class="w-100" rows="3" placeholder="say something about this workout"></textarea>
                    </div>
                    <div class="mt-3 ">
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="w-80">
                                ...
                                <!-- <button v-if="showSuperset" class="prim_btn py-1 brds-2 px-4 me-2 mb-1" @click="superSetSelection()">Superset</button>
                                <button v-if="showCircuit" class="prim_btn py-1 brds-2 px-4" @click="circuitSelection()">Circuit</button>
                                <button v-else class="scnd_btn brds-2 ms-2 py-1" @click="removeFromWorkout()">Remove</button> -->
                    ...
                    <div class="mt-3 w-100" @dragenter.prevent @dragover.prevent>
                        ...
                        <div class="position-relative" v-for="(item, index) in addedExs" draggable="true"
                            ...
                            <div class="position-absolute" style="width:20px;left:-10px;top:calc(50% - 25px);">
                                <button @click="moveUp(index)" v-if="addedExs.length>1 && index!=0" class="move-up tsl" title="Move Up"><i class="fa-sharp fa-solid fa-arrow-up"></i></button>
                                <button @click="moveDown(index)" v-if="addedExs.length>1 && index+1<addedExs.length" class="move-down tsl" title="Move Down"><i class="fa-solid fa-arrow-down"></i></button>
                            </div>
                            <!-- <input v-if="workout.type !='circuit'" @change="supCirBtns()" type="checkbox" class="form-check-input ExCheck" :value="index" v-model="selectedExs"> -->
                            ...
                            <div v-if="item.type=='simple'" class="shd_card mt-4 exs p-2 wrk-ex">
                                ...
                                <div class="col-12 d-flex mt-2">
                                    <div class="singleBox">
                                        <div class="text-center" v-if="item.item.exercise_id!=null">
                                            <img :src="item.item.tempUrl" alt="" class="img-fluid" style="max-height:75px;">
                                            <p class="mb-0"><strong>{{item.item.tempTitle.substring(0,15)}}</strong></p>
                                        </div>
                                        <div v-else class="rest-box">Rest</div>
                                    </div>
                                    <div class="singleBox">
                                        <div class="text-center" v-if="item.item.exercise_id!=null">
                                            <input type="number"  min="1" style="width:70px" v-model="addedExs[index].item.sets">
                                        </div>
                                    </div>
                                    <div class="singleBox">
                                        <div class="text-center position-relative" v-if="item.item.exercise_id!=null">
                                            ...
                                            <div class="kkkk" v-if="item.item.reps_type=='time'">
                                                <select v-model="addedExs[index].item.time">
                                                    <option :value="timeOp" selected v-for="timeOp in timeOptions">{{timeOp}}</option>
                                                </select>
                                            </div>
                                            <div class="text-center">
                                                <input type="text" class="py-1 px-2 brds-1 tm-brdr w-100" v-model="addedExs[index].item.reps">
                                            </div>
                                        </div>
                                    </div>
                                        <div class="singleBox">
                                        <div class="text-center w-100"  v-if="workout.type=='regular' || item.item.exercise_id==null">
                                            <select v-model.number="addedExs[index].item.rest_period">
                                ...
                                <div class="col-12">
                                    <textarea v-model="addedExs[index].item.description" placeholder="Enter instructions for the exercise" class="w-100 mt-2" rows="1"></textarea>
                                </div>
                            </div>
                            <div v-else class="shd_card mt-4 wrk-ex">
                                <div class="ms-4 d-flex justify-content-between group-head">
                                    <!-- <span class="">{{item.type_name}} of: <input style="width:60px" type="number" min="1" v-model="addedExs[index].sets_rounds"> Rounds</span> -->
                                ...
                                <div v-for="(item2, index2) in item.items" class="mt-4 exs p-2 tm-brdr brds-2">
                                    ...
                                    <div class="col-12 d-flex mt-2">
                                        <div class="singleBox">
                                            <div class="text-center" v-if="item2.exercise_id!=null">
                                                <img :src="item2.tempUrl" alt="" class="img-fluid" style="max-height:75px;">
                                                <p class="mb-0"><strong>{{item2.tempTitle.substring(0,15)}}</strong></p>
                                            </div>
                                            <div v-else class="rest-box">Rest</div>
                                        </div>
                                        <div class="singleBox">
                                            <div class="text-center" v-if="item2.exercise_id!=null">
                                                <input type="number"  min="1" style="width:70px" v-model="addedExs[index].items[index2].sets" >
                                            </div>
                                        </div>
                                        <div class="singleBox">
                                            <div class="text-center position-relative" v-if="item2.exercise_id!=null">
                                                ...
                                                <div class="kkkk" v-if="item2.reps_type=='time'">
                                                    <select v-model="addedExs[index].items[index2].time">
                                                        <option :value="timeOp" selected v-for="timeOp in timeOptions">{{timeOp}}</option>
                                                    </select>
                                                </div>
                                                <div class="text-center">
                                                    <input type="text" class="py-1 px-2 brds-1 tm-brdr w-100" v-model="addedExs[index].items[index2].reps">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="singleBox">
                                            <div class="text-center w-100"  v-if="workout.type=='regular' || item2.exercise_id==null">
                                                <select v-model.number="addedExs[index].items[index2].rest_period">
                                                    <option v-for="opt in restPeriodOptions" :key="opt.value" :value="opt.value">{{opt.label}}</option>
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-12">
                                        <textarea v-model="addedExs[index].items[index2].description" placeholder="Enter instructions for the exercise" class="w-100 mt-2" rows="1"></textarea>
                                    </div>
                                </div>
                            </div>
                            <!-- <div v-if="item.type=='circuit'" class="shd_card mt-4 wrk-ex">
                                <div class="ms-4 d-flex justify-content-between group-head">
                                    <span class="">Circuit of: <input style="width:60px" type="number" min="1" v-model="addedExs[index].rounds"> Rounds</span>
                                    <button v-if="workout.type!='circuit'" class="prim_btn brds-2 py-1" @click="ungroup(index)">Ungroup</button>
                                </div>
                                <div v-for="(item2, index2) in item.items" class=" mt-4 p-2 exs tm-brdr brds-2">
                                    <div v-if="workout.type=='circuit'" class="position-absolute" style="width:20px;left:-10px;top:calc(50% - 25px);z-index:9999;">
                                        <button @click="moveUp(index2,'circuit')" v-if="addedExs[0].items.length>1 && index2!=0" class="move-up tsl" title="Move Up"><i class="fa-sharp fa-solid fa-arrow-up"> </i></button>
                                        <button @click="moveDown(index2,'circuit')" v-if="addedExs[0].items.length>1 && index2+1<addedExs[0].items.length" class="move-down tsl" title="Move Down"><i class="fa-solid fa-arrow-down"> </i></button>
                                    </div>
                                    <div class="gray_bg px-3 py-1 mb-1">
                                        <div class="d-flex justify-content-around">
                                            <span class="heading">Exercise</span>
                                    ...
                                    <div class="col-12 d-flex mt-2">
                                        <div class="singleBox">
                                            <div class="text-center position-relative" v-if="item2.exercise_id!=null">
                                                <input v-if="workout.type =='circuit'" type="checkbox" class="form-check-input position-absolute" style="left:5px;" :value="index2" v-model="selectedExsCirc">
                                                <img :src="item2.tempUrl" alt="" class="img-fluid" style="max-height:75px;">
                                                <p class="mb-0"><strong>{{item2.tempTitle}}</strong></p>
                                            </div>
                                            <div v-else class="rest-box">
                                                <input v-if="workout.type =='circuit'" type="checkbox" class="form-check-input position-absolute" style="left:12px;top:7px;" :value="index2" v-model="selectedExsCirc">
                                                Rest
                                            </div>
                                        </div>
                                        <div class="singleBox">
                                            <div class="text-center" v-if="workout.type=='regular' && item2.exercise_id!=null">
                                                <input type="number"  min="1" style="width:70px" v-model="addedExs[index].items[index2].sets" >
                                            </div>
                                        </div>
                                        <div class="singleBox">
                                            <div class="text-center position-relative" v-if="item2.exercise_id!=null">
                                                <div> this sections is commented
                                                    <select class="fawsm-unicode" @change="toggleTimeText($event)">
                                                        <option value="time">&#xf017;</option>
                                                        <option value="text">&#xf037;</option>
                                                    </select>
                                                </div>  till here
                                                <div class="text-center">
                                                    <input type="number" class="py-1 px-2 brds-1 tm-brdr w-50" v-model="addedExs[index].items[index2].reps">
                                                </div>
                                                <div class="kkkk">
                                                    these five lines are commented
                                                    <select v-model="addedExs[index].items[index2].time">
                                                        <option :value="timeOp" selected v-for="timeOp in timeOptions">{{timeOp}}</option>
                                                    </select>
                                                    <Select2MultipleControl  @change="myChangeEvent($event,index,2,index2)" :options="exerciseTarget"/>
                                                    <p v-if="addedExs[index].items[index2].target.length<1" class="position-absolute mb-0" style="bottom:0;font-size:13px;pointer-events:none;">Enter Target Muscle</p>
                                                    <select v-model="addedExs[index].items[index2].target" style="width:80%" name="" id="" >
                                                        <option v-for="(item, index) in exerciseTarget" :key="index" :value="item">{{item}}</option>
                                                    </select>
                                                    <input  type="text" placeholder="target, muscle etc" style="width:80%;color: rgb(192, 192, 192);background-color: white;border-radius: 5px;border: 1px solid rgb(197, 197, 197);padding: 3px 7px;">
                                                    till here
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="col-xl-6 mt-5 mt-xl-0">
                    <div class="shd_card p-2 mb-3">
                        ...
                        <div class="d-flex flex-wrap brds-1 p-2 mt-2 border" style="max-height:90px;overflow-y:auto">
                            <span v-for="tag in workout.tagNames" class="px-2 py-1 prim_bg mx-2 brds-1 my-1">{{tag}}</span>
    ...
    methods: {
        ...
        onDropSort1(event, droppedItem){
            if(event.dataTransfer.getData('itemOrder')){
                ...
                const itemPosition = this.addedExs.findIndex((item) => item.order == itemOrder)
                const droppedItemPosition = this.addedExs.findIndex((item) => item.order == droppedItem.order)
        ...
        updateOrder(){
            for (let index = 0; index < this.addedExs.length; index++) { 
                this.addedExs[index].order = index;
            }
        },
        applyFilters(tagIds){
        ...
        quitComponent() {
            this.loadingText = 'Quiting';
            this.loading = true;
            axios
                .get(
                    config.baseApiUrl + "delete-workout/" + this.workout.id,
                    this.apiConfig
                )
                .then((res) => {
                    this.loading = false;
                    if (res.data.status) this.$parent.closeWorkoutBuilder(null);
                    else {
                        this.modalTitle = "Error!";
                        this.modalDetail = res.data.message;
                        this.informModal = true;
                        console.log("Workout builder quit error", res.data.message);
                    }
                })
                .catch((er) => {
                    ...
                    this.informModal = true;
                    console.log("Workout builder quit error", er.error);
                });
        },
        getExercises() {
            ...
            (this.loading = true),
                axios
                    .get(config.baseApiUrl + "get-all-exercises?lang=" + this.workout.language, this.apiConfig)
                    .then((res) => {
                        this.loading = false;
                        if (res.data.status){
        ...
        addRest() {
            ...
            if (this.workout.type !== "circuit") {
                this.addedExs.push(tempData);
            }
            else {
                if (this.addedExs.length == 0) {
                    let tempParent = {
                        type: "circuit",
                        rounds: 1,
                        items: [],
                    };
                    this.addedExs.push(tempParent);
                }
                this.addedExs[0].items.push(tempData.item)
            }
            this.supCirBtns();
        },
        validate() {
            let restCheck = false;
            if (this.addedExs.length == 0) {    // check for no exercise added
                this.modalTitle = "Error!";
                this.modalDetail = "No exercise added please add first";
                this.informModal = true;
                return;
            }
            for (let index = 0; index < this.addedExs.length; index++) {    // check if only rests added
                if (this.addedExs[index].type !== 'simple') {
                    for (let index1 = 0; index1 < this.addedExs[index].items.length; index1++) {
                        if (this.addedExs[index].items[index1].exercise_id !== null) {
                            restCheck = true;
                            break
                        }
                    }
                    ...
                    if (this.addedExs[index].item.exercise_id !== null) {
                        restCheck = true;
                        break
                    }
                }
            ...
            // if (this.instructions == null || this.instructions == "") {     // check empty instrructions
            //     this.modalTitle = "Error!";
            //     this.modalDetail = "No instructions added please add first";
            //     this.informModal = true;
            //     return;
            // }
            this.addedExs.forEach((item1) => {
        ...
        upload(){
            ...
            this.loading = true;
            axios.post(config.baseApiUrl + "create-workout-exercise", postData, this.apiConfig)
            .then((res) => {
                this.loading = false;
                if (res.data.status) {
                    this.modalTitle = "Done!";
                    this.modalDetail = "Workout saved";
                    this.informModal = true;
                    this.$parent.closeWorkoutBuilder(res.data.workout);
                } else {
                    this.modalTitle = "Error!";
                    this.modalDetail = res.data.message;
                    this.informModal = true;
                    console.log("Posting workout error", res.data.message);
                }
            })
            .catch((er) => {
                this.loading = false;
        ...
        ungroup(index) {
            let tempArray = this.addedExs[index].items;
            this.addedExs.splice(index, 1);
            for (let i = 0; i < tempArray.length; i++) {
                const element = {
                    type: "simple",
                    item: tempArray[i],
                };
                this.addedExs.splice(index + i, 0, element);
            }
        },
        moveUp(m, n) {
            let index = m;
            let type = n;
            if (type == "circuit") {
                let temp = this.addedExs[0].items[index];
                this.addedExs[0].items[index] = this.addedExs[0].items[index - 1];
                this.addedExs[0].items[index - 1] = temp;
            } else {
                let temp = this.addedExs[index];
                this.addedExs[index] = this.addedExs[index - 1];
                this.addedExs[index - 1] = temp;
            }
        },
        moveDown(m, n) {
            let index = m;
        ...
        //         selTypes.push(this.addedExs[this.selectedExs[index]].type);
        //     }
        //     this.selectedExs = this.selectedExs.sort();
        ...
        //         let tempItems = [];
        //         for (let index = 0; index < this.selectedExs.length; index++) {
        //             tempItems.push(this.addedExs[this.selectedExs[index]].item); //adding all selected exercise to single array
        //         }
        //         let tempData = {
        ...
        //         for (let index = 0; index < this.selectedExs.length; index++) {
        //             this.addedExs.splice(this.selectedExs[index] - index, 1); // deleting selected indexes
        //         }
        //         let tempIndex = this.selectedExs.sort()[0];
        //         this.addedExs.splice(tempIndex, 0, tempData); // add grouped data to post data
        //     } else if (
        ...
        //             let itemsToPush = this.addedExs[this.selectedExs[index]].items; //get exercises in the group
        //             this.addedExs[tempIndex].items.push(...itemsToPush); // push into first one
        //             this.addedExs.splice(this.selectedExs[index], 1); // delete group after adding in first
        //         }
        //     } else if (
        ...
        //             if (tempIndex == null && this.addedExs[element].type == "superset") {
        ...
        //             if (element != tempIndex && this.addedExs[element].type == "simple") {
        //                 // if current item is simple
        //                 let itemToadd = this.addedExs[element].item;
        //                 if (element > tempIndex)
        //                     // if located after superset
        //                     this.addedExs[tempIndex].items.push(itemToadd);
        //                 // add to superset at end
        //                 // else loacated before superset
        //                 else this.addedExs[tempIndex].items.splice(0, 0, itemToadd); // add to superset at start
        //                 indexesToDel.push(element); // delete that item after adding
        //             } else if (
        //                 element != tempIndex &&
        //                 this.addedExs[element].type == "superset"
        //             ) {
        //                 // if current item is a superset group
        //                 let tempItems = this.addedExs[element].items;
        //                 this.addedExs[tempIndex].items.push(...tempItems); // add to superset
        //                 indexesToDel.push(element); // delete that item after adding
        //             }
        //         }
        //         for (let j = 0; j < indexesToDel.length; j++) {
        //             const elem = indexesToDel[j];
        //             this.addedExs.splice(elem, 1);
        //         }
        //     }
        ...
        //         for (let index = 0; index < this.selectedExs.length; index++) {
        //             const element = this.selectedExs[index];
        //             if (tempIndex == null && this.addedExs[element].type == "circuit") {
        //                 tempIndex = element; //getting index of first superset in the selection
        //             }
        //         }
        //         let indexesToDel = [];
        ...
        //             } else if (
        //                 element != tempIndex &&
        //                 this.addedExs[element].type == "circuit"
        //             ) {
        //                 // if current item is a superset group
        //                 let tempItems = this.addedExs[element].items;
        //                 this.addedExs[tempIndex].items.push(...tempItems); // add to superset
        ...
        //     let exerciseIds = [];
        //     let error = false;
        //     if (this.workout.type == "regular") {
        //         // regular workout type
        //         if (this.addedExs.length > 1) {
        ...
        //                         exerciseIds.push(this.addedExs[this.selectedExs[index]].item.exercise_id);
        //                     }
        //                 }
        ...
        catSelected(cat){
            ...
            let tempIndex = this.selectedExs.sort()[0];
            this.addedExs.splice(tempIndex, 0, tempData); // add grouped data to main data
